...
##clean_lineitems <- clean_lineitems %>% 
 # group_by(id_order) %>% 
  #summarise_at(tbl_vars(product_quantity.x, totalpaid_estimate, total_paid.x, total_diff), funs(mean))
...
#Inverstigating frequency of products by top 20 products
...
barplot(sort(itemFrequency(sku_order), decreasing=F))
